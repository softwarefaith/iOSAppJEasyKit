...
+ (NSString *)networkType{
    ...
    MLReachability *reach = [MLReachability reachabilityWithHostName:@"www.baidu.com"];
...
{// 需要#import "sys/utsname.h"
    ...
    if ([deviceString isEqualToString:@"iPhone8"])         return @"iPhone 7";
    ...
    // Apple TV
    if ([deviceString hasPrefix:@"AppleTV2"])           return @"Apple TV 2G";
    if ([deviceString hasPrefix:@"AppleTV3"])           return @"Apple TV 3G";
    
    if ([deviceString isEqualToString:@"i386"])         return @"Simulator";
    if ([deviceString isEqualToString:@"x86_64"])       return @"Simulator";
    
    
    if ([deviceString hasPrefix:@"iPhone"])             return  @"Unknown iPhone";
    if ([deviceString hasPrefix:@"iPod"])               return  @"Unknown iPod";
    if ([deviceString hasPrefix:@"iPad"])               return  @"Unknown iPad";
    if ([deviceString hasPrefix:@"AppleTV"])            return  @"Unknown Apple TV";
...
+ (NSString *)kernelVersion {
    return [self utsSystemInfo][kRelease];
}

+ (NSString *)darwinBuildDescription {
    return [self utsSystemInfo][kVersion];
}
...
+ (NSDictionary *)utsSystemInfo {
    
    static NSDictionary *share = nil;
    if (!share) {
        struct utsname systemInfo;
        uname(&systemInfo);
        share = @{kSysName:[NSString stringWithCString:systemInfo.sysname encoding:NSUTF8StringEncoding],kNodeName:[NSString stringWithCString:systemInfo.nodename encoding:NSUTF8StringEncoding],kRelease:[NSString stringWithCString:systemInfo.release encoding:NSUTF8StringEncoding],kVersion:[NSString stringWithCString:systemInfo.version encoding:NSUTF8StringEncoding],kMachine:[NSString stringWithCString:systemInfo.machine encoding:NSUTF8StringEncoding]};
    }
    return share;
}
